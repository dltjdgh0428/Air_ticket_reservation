...
public class JpaTicketRepository implements MyTicketRepository,BuyTicketRepository,TicketRepository{
    ...
    public ReservationEntity saveReservation(ReservationEntity reservation) {
        ...
        ReservationEntity savedReservation = em.find(ReservationEntity.class, reservation.getReservation_id());
    ...
    public List<JourneyEntity> findCond(Long d_code, Long a_code, String d_time, String a_time) {
        ...
        List<JourneyEntity> result;

        if(d_code == 0 && a_code == 0) {
            result = findAllJourney();
        } else {
    ...
    @Override
    public List<ReservationEntity> findMyReservation(String passport) {
        List<ReservationEntity> result;

        String jpql = "select r from reservation r where r.passport LIKE concat(:inputPassport)";
        TypedQuery<ReservationEntity> query = em.createQuery(jpql, ReservationEntity.class);

        query.setParameter("inputPassport", passport);
        result = query.getResultList();
     ...
     * */
    @Override
    public JourneyEntity findCondJourney(Long journeyId) {
        String jpql = "select j from journey j where j.journey_id = :inputId";
        TypedQuery<JourneyEntity> query = em.createQuery(jpql, JourneyEntity.class);
        query.setParameter("inputId", journeyId);
        JourneyEntity result = query.getSingleResult();
        return result;
    }
